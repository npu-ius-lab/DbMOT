...
{
	track_thresh = 0.75;//0.3
	high_thresh = 0.6;
	match_thresh = 0.8;//0.8

	frame_id = 0;
	max_time_lost = track_buffer; //max age
}
 ...
 std::vector<STrack> BYTETracker::update(const  std::vector<detect_result>& objects)
{
	...
	linear_assignment(dists, dist_size, dist_size_size, match_thresh, matches, u_track, u_detection);//match_thresh
	...
	u_track.clear();
	u_detection.clear();
	linear_assignment(dists, dist_size, dist_size_size, 0.5, matches, u_track, u_detection); //0.5
	...
	u_detection.clear();
	linear_assignment(dists, dist_size, dist_size_size, 0.7, matches, u_unconfirmed, u_detection);//0.7
