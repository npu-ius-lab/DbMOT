...
protected:
    ...
    //* my func
    Size2f get_target_size_() CV_OVERRIDE;
    Rect2d get_search_zone_() CV_OVERRIDE;
    Size2i get_scale_size_() CV_OVERRIDE;
...
Mat TrackerCSRTImpl::calculate_response(const Mat &image, const std::vector<Mat> filter)
{
    ...
    // std::cout << object_center << std::endl;
    
    ////modified////
    int img_rows = image.rows;
    int img_cols = image.cols;
    Rect2d search_zone = get_search_window(object_center, cvFloor(current_scale_factor * template_size.width),
        cvFloor(current_scale_factor * template_size.height), img_rows, img_cols);
    this->search_roi = search_zone;
    this->scale_size.width = cvFloor(current_scale_factor * template_size.width); //搜索区域的宽高
    this->scale_size.height = cvFloor(current_scale_factor * template_size.height);
    ////modified////
...
bool TrackerCSRTImpl::updateImpl(const Mat& image_, Rect2d& boundingBox)
{
    ...
    if (object_center.x < 0 && object_center.y < 0) 
    {
        object_center.x = boundingBox.x + original_target_size.width / 2.0f;
        object_center.y = boundingBox.y + original_target_size.height / 2.0f;
        ...
        if (object_center.x >= image.cols)
            object_center.x = image.cols - cvFloor(current_scale_factor * template_size.width) / 2.0f - 1;
        
        if (object_center.y >= image.rows)
            object_center.y = image.rows - cvFloor(current_scale_factor * template_size.height) / 2.0f - 1;
    }
  ...
  ////modified////
    this->ori_target_size.width = current_scale_factor * original_target_size.width;
    this->ori_target_size.height = current_scale_factor * original_target_size.height;
  ////////////////
...
Size2f TrackerCSRTImpl::get_target_size_()
{
    return this->ori_target_size;
}

Rect2d TrackerCSRTImpl::get_search_zone_()
{
    return this->search_roi;
}

Size2i TrackerCSRTImpl::get_scale_size_()
{
