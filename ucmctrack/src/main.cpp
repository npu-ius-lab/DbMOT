...
cv::Rect2d tranScale(darknet_ros_msgs::BoundingBox &bbx_msg,
...
{
    pub = n.advertise<visualization_msgs::MarkerArray>("ucmc_track/kf_predictions", 10);
...
{
    frameCount++;
    ros::Time timeStamp = img_ptr->header.stamp;
    ...
    for (int i = 0; i < detect_msg->bounding_boxes.size(); i++)
    {
        ...
        PROJ_RESULT proj_res = proj.pixel2world(bbx);
    ...
    vector<track_data> res = mytracker.update(detections, frameCount);

    for (Detection& detection : detections) {
        if (detection.trackid > 0) {
            cv::rectangle(img, detection.boundingbox, cv::Scalar(0,255,0), 2);
            cv::putText(img, cv::format("%d", detection.trackid), cv::Point(
                detection.boundingbox.x, detection.boundingbox.y - 5), 0, 0.6, cv::Scalar(255,255,0), 2, cv::LINE_AA);
                
            // std::cout << frameCount << "," << detection.trackid << "," << detection.boundingbox.x << "," 
            //     << detection.boundingbox.y << "," << detection.boundingbox.width << "," << detection.boundingbox.height <<
            //     ",1,-1,-1,-1"<< std::endl;
        }
    }
    ...
    for (track_data& p : res) 
    {
        ...
        m.pose.position.x = p.kf_pred.x;
        m.pose.position.y = p.kf_pred.y;
        m.pose.position.z = 0;
        ...
        n.ns = "target_pos" + to_string(p.id);
        n.id = p.id;
        n.type = Marker::ARROW;
        n.pose.position.x = p.kf_pred.x;
        n.pose.position.y = p.kf_pred.y;
        n.pose.position.z = 0;
