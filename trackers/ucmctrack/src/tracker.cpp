...
vector<track_data> UCMCTrack::update(vector<Detection> &detections, int count)
{
    data_association(detections);
    associate_tentative(detections);
    initial_tentative(detections);
    delete_old_trackers();
    update_status(detections);
    vector<track_data> ret;
    for (Track& track : trackers) 
    {
        if (track.is_active() || count <= min_hits) {
            track_data td;
            td.id = track.id;
            td.kf_pred = track.kf_pred;
            ret.push_back(td);
        }
    }
    return ret;
...
void UCMCTrack::data_association(vector<Detection> &detections)
{   
    ...
    for (Track& track : trackers) 
    {
        State3D res = track.predict(this->kf);
        track.kf_pred = res;
    }
    ...
    {   
        ...
        // cout << "1st:  " << "m: " << matches.size() << "  ud:" << u_detection.size() << "  ut:" << u_track.size() << endl;
    ...
    {   
        ...
        for (int i=0; i<num_trk; i++) {
         ...
         std::vector<int> u_track, u_detection;
        linear_assignment(cost_matrix, num_trk, num_det, mahal_thresh, matches, u_track, u_detection);
        // cout << "2nd:  " << "m: " << matches.size() << "  ud:" << u_detection.size() << "  ut:" << u_track.size() << endl;
        for (int idx : u_track) {
...
void UCMCTrack::delete_old_trackers()
{
    for (auto it=trackers.begin(); it!=trackers.end();)
    {
        ...
        // bool is_missed = track.time_since_update >= max_age;
...
void UCMCTrack::update_status(vector<Detection> &detections)
{
    ...
    for (int i=0; i<trackers.size(); i++)
    {
        ...
        if ((detidx >= 0) && (detidx < detections.size())) {
            track.roi_w = detections[track.detidx].boundingbox.width;
            track.roi_h = detections[track.detidx].boundingbox.height;
        }
        if (track.is_active()) {
