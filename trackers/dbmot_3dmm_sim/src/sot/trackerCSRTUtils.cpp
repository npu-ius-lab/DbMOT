...
Rect2d get_search_window(const Point2f center, const int w, const int h, const int img_rows, const int img_cols)
{
    int startx = cvFloor(center.x) + 1 - (cvFloor(w/2));
    int starty = cvFloor(center.y) + 1 - (cvFloor(h/2));
    Rect roi(startx, starty, w, h);
    ...
    roi.width -= padding_left;
    roi.height-= padding_top;
    if(roi.x + roi.width >= img_cols) {
        padding_right = roi.x + roi.width - img_cols;
        roi.width = img_cols - roi.x;
    }
    if(roi.y + roi.height >= img_rows) {
        padding_bottom = roi.y + roi.height - img_rows;
        roi.height = img_rows - roi.y;
    }
    return roi;
}
